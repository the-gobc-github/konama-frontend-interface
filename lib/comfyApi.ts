...
export const ComfyAPI = {
    ...
    async updateInstance(instanceId: string, params: UpdateInstanceParams): Promise<SingleComfyInstanceResponse> {
        const response = await authFetch.patch(`${BASE_URL}/comfy/instances/${instanceId}`, params);
        return response.data;
    },

    // Scan workflows
    async scanWorkflows(instanceId: string, workflowName: string, overwrite: boolean = true): Promise<any> {
        const response = await authFetch.post(`${BASE_URL}/comfy/instances/${instanceId}/workflows/scan?stream=true`, {
            name: workflowName,
            overwrite
        });
        return response.data;
